---
title: "Modelength_combi Selection"
author: "Loïc Pages"
date: "`r Sys.Date()`"
output:
...
IPM_data <- read.csv("newdata.csv") #donnees format classique
...
#variance résiduelle pour la croissance
...
Combinaisons pour fécondité qui fixe l'intercept à 0
```{r}
...
result <- foreach(i = 1:length_combi, .packages=c("splines","spaMM"), .combine = cbind) %dopar% {
    set_effects <- combi[i]
...
```{r}
Cptlglm2 <- fitme(Capitule ~ 1 + bs(Size0Mars,df=4,degree=2) + bs(Age,degree=3,knots=c(1.5,6.5)) + (Size0Mars|Pop) + (Size0Mars + Age|year), 
                  data=cptldata, family= Poisson(trunc=0L))

Cptlglm1 <- fitme(log(1+Capitule) ~ 1  + Size0Mars + (Age|year), 
                  data=cptldata)
...
Pltglm2 <- fitme(I(2*Size0Mars) ~ 1 + (1|year) + (1|Pop) + (1|Pop:year), 
                 data=plantule_data,
                 family = poisson(log))
```
...
n <- length(Estb$year)
length_combi <- length(combi_estb)
...
Estb$year1 <- Estb$year+1
essai <- fitmv(submodels=list(list(as.integer(Capitule) ~ 1+(1|Pop:year),family=poisson()),
                              list(NombrePlantules ~ 1+(1|Pop:year1)+(1|year1),family=poisson())), 
                  data=Estb)
```
...
foo <- predict(essai)
mv <- attr(foo,"mv")
Estb$predratio <- mv[[2]]/mv[[1]]
 
    ggplot(data=Estb, aes(x = year, y = predratio)) +
