...
setwd("/media/loic/Commun/0Travail/2025 IPM Centauree ISEM/2025 Stage ISEM/Code/Fitted Models")

IPM_data <- read.csv("/media/loic/Commun/0Travail/2025 IPM Centauree ISEM/2025 Stage ISEM/Code/newdata.csv")
...
plot_surviebis <- function(data = fake_data2, prediction, var, c1, valc1 = 1, c2, valc2 = "Au", fact) {
  data %>%
    ...
    filter(!!sym(c1) == valc1, !!sym(c2) == valc2) %>%
    ggplot(aes(x = .data[[var]], y = surv_predi)) +
    geom_count(data=survdata2,aes(x=Age,y=Survie),alpha = 0.5,show.legend=FALSE) +
    geom_line(aes(color = as.factor(.data[[fact]])),show.legend=FALSE) +
    theme_minimal() +
    ylim(0, 1)
}
...
```{r}
fake_data %>%
    mutate(cptl_predi = predict(Cptlglm2,newdata = fake_data)) %>%
...
Cptlglm2$data %>%
    mutate(cptl_predi = predict(Cptlglm2)) %>%
    group_by(Size0Mars,Pop) %>% 
    summarise(cptl_predi = mean(cptl_predi)) %>%
    ggplot(aes(x = Size0Mars, y = cptl_predi)) +
    geom_point(data = cptl_data, aes(y = Capitule), alpha=0.6)+
