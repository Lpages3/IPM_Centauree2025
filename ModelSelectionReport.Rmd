...
Growthpredict2b <- predict(Growthglm2b)[,1]
```

```{r}
Growthglm3 <- fitme(Size1Mars ~ 1 +
                      poly(Size0Mars,3) + bs(Age,degree=2,knots=6.5),
                    resid.model = ~ poly(Size0Mars,3),
                    data=growthdata)
...
```{r}
growthdata %>%
  mutate(s1_predi = Growthpredict2b**(33/14)) %>%
  ggplot(aes(x = Size1Mars)) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "black") +
  geom_point(aes(y=s1_predi, color=as.factor(Age)),show.legend = FALSE,alpha=0.3) +
  # geom_point(aes(y=s1_obs),color="blue") +
    labs(x = "Observed size(t+1)",
      y = "Predicted size(t+1)",
      fill = "Age",
      color = "Age")+
    scale_color_brewer(palette = "Spectral", direction = -1)+
  theme_bw() +
...
growthdata %>%
  mutate(s1_predi = predict(Growthglm2)[,1]) %>%
  ggplot(aes(x = Size1Mars)) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "black") +
  geom_point(aes(y=s1_predi, color=as.factor(Age)),show.legend = FALSE,alpha=0.3) +
  # geom_point(aes(y=s1_obs),color="blue") +
...
growthdata %>%
  mutate(s1_predi = predict(Growthglm2gamma)[,1]) %>%
  ggplot(aes(x = Size1Mars)) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "black") +
  geom_point(aes(y=s1_predi, color=as.factor(Age)),show.legend=FALSE, alpha=0.3) +
  # geom_point(aes(y=s1_obs),color="blue") +
...
growthdata %>%
  mutate(s1_predi = predict(Growthglm3)[,1]) %>%
  ggplot(aes(x = Size1Mars)) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "black") +
  geom_point(aes(y=s1_predi, color=as.factor(Age)),show.legend=FALSE, alpha=0.3) +
  # geom_point(aes(y=s1_obs),color="blue") +
...
```
```{r}
fake_data %>% mutate(sdgrow = exp(1.747220+24.3401907*Size0Mars-1.8212391*Size0Mars**2+0.3563595*Size0Mars)) %>% 
  group_by(Size0Mars, Age) %>%
...
fake_dataEstb %>%
  ...
  group_by(Capitule,Pop) %>% 
  mutate(plt = mean(plt)) %>% 
  ggplot(aes(x = Capitule, y = plt)) +
  geom_line(aes(color=as.factor(Pop))) +
  theme_bw()+
  labs(x = "Year",
