...
#                       poly(sqrt(Size0Mars),4) + poly(age0,3) + 
#                       (sqrt(Size0Mars)|year) + (sqrt(Size0Mars)|Pop),
#                     data=growthdata)

cb <- MASS::boxcox(Size1Mars ~ 1 + 
                      poly(sqrt(Size0Mars),4) + poly(age0,3) , data=growthdata)
cb$x[cb$y==max(cb$y)]
...
$$logit(s_1(x))=\beta_0+\gamma_0+(\beta_1+\epsilon_1)x+\beta_2x^2+\beta_3x^3$$

$$logit(s_1(x))=-1.51+\gamma_0+\epsilon_0+(36.02+\epsilon_1)x+12.04x^2+29.46x^3$$
...
$\gamma_0 \sim \mathcal{N}(0,0.89)$ : contribution de l'effet aléatoire Population sur l'intercept

$\epsilon_j$, $j\in\{0,1\}$: contribution de l'effet aléatoire Année sur l'intercept et la taille
$\mathcal{E} \sim \mathcal{N}(0,\Sigma)$ with $$\Sigma =
  \left[ {\begin{array}{cc}
    1.57 & -0.78 \\
    -0.78 & 0.066 \\
  \end{array} } \right]$$
...
Splines effets fixes :
$$logit(s_a(x))=2.56\times10^{-16} + 1.28(x - 0.5) - 0.142(x - 0.5) ^ 2  +2.31 + 0.569(x - 3) - 0.0708(x - 3) ^ 2  +3.38 + 0.144(x - 6) - 0.00504(x - 6) ^ 2 +3.33\times10^{-15} + 0.612  (a - 2) - 0.416 (a - 2) ^ 2 + 0.0617 (a - 2) ^ 3 -0.0382 + 0.621  (a - 6.5) + 0.418(a - 6.5) ^ 2 - 0.844(a - 6.5) ^ 3$$

$$logit(s_a(x)=243.61+\gamma_0+\epsilon_0+2.62x-0.218x^2-(108.77+\epsilon_1)a+16.09a^2 -0.78a^3$$
avec :
...
$\gamma_0 \sim \mathcal{N}(0,0.073)$ : contribution de l'effet aléatoire Population sur l'intercept

$\epsilon_j$, $j\in\{0,1\}$: contribution de l'effet aléatoire Année sur l'intercept et l'age
$\mathcal{E} \sim \mathcal{N}(0,\Sigma)$ with $$\Sigma =
  \left[ {\begin{array}{cc}
    1.39 & -0.91 \\
    -0.91 & 0.0097 \\
...
$$logit(f_a(x))=\beta_0+\gamma_0+\beta_{1,1}x+\beta_{1,2}x^2+\beta_{1,3}x^3+(\beta_{2,1}+\gamma_2)a+\beta_{2,2}a^2$$

$$logit(f_a(x))=11.49+2.17+258.56x-86.88x^2+42.73x^3+(148.26+0.09)a-57.46a^2$$
avec :
...
$\gamma_0$ : contribution de l'effet aléatoire Population sur l'intercept

$\gamma_2$ : contribution de l'effet aléatoire Population sur l'age

$\Gamma \sim \mathcal{N}(0,\Sigma)$ with $$\Sigma =
  \left[ {\begin{array}{cc}
    2.17 & -0.98 \\
    -0.98 & 0.09 \\
  \end{array} } \right]$$
...
$y$ : taille de la plantule au temps

$\beta_0$ : intercept fixe

$\gamma_0$ : effet aléatoire Population

$\epsilon_j$ : effet aléatoire Année

$\delta_0$ : effet aléatoire de l'intéraction Population:Année
...
Gyx0 <- function (y, x, a) {
  ...
  return(dnorm(y, mean = exp(sortie2$V1), sd = exp(Sdev))) #Proba qu'un individu soit de taille y à t+1 sachant la moyenne et la variance
}

data_predi_growth <- fake_data3 %>%
  filter(Pop=="Au",year==2000) %>% 
  select(-year,-Pop) %>% 
  group_by(Size0Mars,age0,Size1) %>% 
  mutate(prob = Gyx0(x=Size0Mars, y=Size1,a=age0))

data_predi_growth %>% 
  ggplot(aes(x = Size0Mars, y = Size1)) +
...
#Survie plantules
resSurv1 <- get_predVar(Survglm11,newdata = fake_data1)
...
Survglm11 <- fitme(SurvieMars ~ 1+ poly(Size0Mars,3) + (1|year) + (1|Pop),
                  ...
                  data=survdata1,
                  method="PQL/L")
extractBIC(Survglm11,ntot=length(survdata1),N=28*6)

Survglm12 <- fitme(SurvieMars ~ 1 + bs(Size0Mars,df=3,degree=2) + (age0|year) + (1|Pop),
                  family=binomial,
                  data=survdata2,
                  method="PQL/L")
extractBIC(Survglm12,ntot=length(survdata2),N=28*6)

Flowglm1 <- fitme(Flowering0 ~  1 + poly(Size0Mars,3) + poly(age0,2) + (age0|Pop),
                 family=binomial,
                 data=centauree_data, method="PQL/L")
extractBIC(Flowglm1,ntot=length(centauree_data),N=6)

Growthglm1 <- fitme(log(Size1Mars) ~ 1 + poly(log(Size0Mars),3) + poly(age0,3) + (log(Size0Mars)|year) + (log(Size0Mars)|Pop),
                    data=growthdata)
extractBIC(Growthglm1,ntot=length(growthdata),N=6*28)

Pltglm1 <- fitme(Size0Mars ~ 1 + (1|year) + (1|Pop) + (1|Pop:year),
                 data=plantule_data,
                 family = Gamma(log))
extractBIC(Pltglm1, ntot = length(plantule_data), N=28*6*147)

Cptlglm1 <- fitme(Cptl0 ~ 1 + Size0Mars,
                  data = cptldata)
extractBIC(Cptlglm1,ntot = length(cptldata),N=1)
```
