...
Grow <- function (x, a) {
  ...
  sortie <- predict(Growthglm1, fake_data, allow.new.levels = T, residVar=TRUE) #Prédit les log(taille) à t+1 possibles 
  sortie2 <- aggregate(sortie, list(fake_data$Size0Mars), mean) #Calcule la moyenne des log(taille)t+1
  Sdev <- sd(sortie)
  return(ifelse(rnorm(1, mean = exp(sortie2$V1), sd = exp(Sdev))<0,0,rnorm(1, mean = exp(sortie2$V1), sd = exp(Sdev)))) #Proba qu'un individu soit de taille y à t+1 sachant la moyenne et la variance
  # return(exp(sortie2$V1))
}
fake_growth <- tibble(
   year = annees[1:8],
   age0 = age_range,
   ...
   size4 = NA)
start <- 2
fake_growth$size0[1] <- start
fake_growth$size1[1] <- start
fake_growth$size2[1] <- start
fake_growth$size3[1] <- start
fake_growth$size4[1] <- start

for (i in 2:8){
  x <- fake_growth$size0[i-1]
  x1 <- fake_growth$size1[i-1]
  x2 <- fake_growth$size2[i-1]
  x3 <- fake_growth$size3[i-1]
  x4 <- fake_growth$size4[i-1]

  a <- i
  fake_growth$size0[i] <- Grow(x,a)
  fake_growth$size1[i] <- Grow(x1,a)
  fake_growth$size2[i] <- Grow(x2,a)
  fake_growth$size3[i] <- Grow(x3,a)
  fake_growth$size4[i] <- Grow(x4,a)

}

ggplot(data = fake_growth, aes(x=age0))+
  geom_line(aes(y=size0),color="red")+
  geom_line(aes(y=size1),color="blue")+
  geom_line(aes(y=size2),color="green")+
  geom_line(aes(y=size3),color="magenta")+
  geom_line(aes(y=size4),color="orange")+
  theme_bw()
```
