...
```{r,include=FALSE}
annees <- 1995:2022
populations <- c("Au","E2","E1","Po","Pe","Cr")
...
fake_data1 %>%
  ...
  mutate(surv_predi = mean(surv, na.rm = TRUE),
         .group="drop") %>%
  ggplot(aes(x = Size0Mars, y = surv_predi)) +
  geom_count(data=survdata1,aes(x=Size0Mars,y=Survie),alpha = 0.4) +
...
fake_data %>%
  ...
  geom_count(data=growthdata,aes(x=Size0Mars,y=Size1Mars),alpha = 0.3,show.legend = FALSE) +
