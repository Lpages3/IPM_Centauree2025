...
i <- 1L
combi_growth2 <- list()
for (set_effects in combi_growth) {
  effect_formula <- paste(unlist(set_effects), collapse = "+")
  formula <- as.formula(paste("Flowering0", "~", effect_formula))
  if (is_separated.formula(formula, data = growthdata,family = gaussian(log)) == FALSE) {
    combi_growth2[[i]] <- set_effects
    i <- i + 1L
  }
  print(i)
}
