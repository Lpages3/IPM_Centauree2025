...
```{r}
Growthglm1 <- fitme(Size1Mars ~ 1 + bs(Size0Mars,df=5,degree=3) + bs(age0,degree=2,knots=6.5)+(age0|year)+(Size0Mars|Pop),
...
result <- foreach(i = 1:ls, .packages=c("splines","spaMM"), .combine = cbind)  %:%
  ...
  c(extractAIC(fit)[2],extractBIC(fit,n))
}
