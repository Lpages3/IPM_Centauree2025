...
size_effects <- c("Size0Mars","poly(Size0Mars,2)","bs(Size0Mars,df=4,degree=2)",
                   "poly(Size0Mars,3)","bs(Size0Mars,df=5,degree=3)","poly(Size0Mars,4)")

size_effects2 <- c("Size0Mars","poly(Size0Mars,2)","poly(Size0Mars,3)",
                   "poly(Size0Mars,4)")

age_effects <- c("age0","poly(age0,2)","bs(age0,degree=2,knots=6.5)",
                   "poly(age0,3)","bs(age0,degree=3,knots=6.5)","bs(age0,degree=3,knots=c(1.5,6.5))",
                   "poly(age0,4)")

age_effects2 <- c("age0","poly(age0,2)","poly(age0,3)",
                   "poly(age0,4)")
```
...
# Ajouter combinaisons avec splines de size
...
# Ajouter combinaisons avec splines de age
...
validNrw <- Filter(is_valid_Nrw,combi) # Ensemble des combinaisons fonctionelles (comprenant l'effet individu)
...
Génération de toutes les combinaisons avec les polynômes
```{r} 
combinations <- list()

# Combinaisons sans "poly" ni "Size0Mars" ni "age0"
for (n in 1:length(random_effects)) {
  combinations <- c(combinations, combn(random_effects, n, simplify = FALSE))
}
...
# Ajouter chaque "poly" de size et age seul et avec d'autres
for (s in size_effects2) {
  for (combo in combi) {
    combinations <- c(combinations, list(c(s, combo)))
  }
}
...
combi_growth <- lapply(valid, function(x) gsub("Size0Mars", "log(Size0Mars)", x))
combi_growthNrw <- lapply(validNrw, function(x) gsub("Size0Mars", "log(Size0Mars)", x))
combi_growth2 <- lapply(valid2, function(x) gsub("Size0Mars", "log(Size0Mars)", x))
...
i <- 1L
combi_surv <- list()
for (set_effects in validNrw) {
  effect_formula <- paste(unlist(set_effects), collapse = "+")
  formula <- as.formula(paste("SurvieMars", "~", effect_formula))
  if (is_separated.formula(formula, data = survdataNot1, method = "PQL/L") == FALSE){
    combi_surv[[i]] <- set_effects
    print(i)
  }
  i <- i + 1L
}
...
survdataNot1 <- survdata[survdata$age0!=1,]
...
```{r}
Growthglm<- fitme(Size1Mars ~ 1 + bs(log(Size0Mars),df =5,degree=3) + bs(age0,degree=3,df=3)+(log(Size0Mars)|year),
