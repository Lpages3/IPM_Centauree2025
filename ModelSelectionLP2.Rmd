...
result <- foreach(i=1:ls, .packages=c("splines","spaMM")) %dopar% {
  ...
  N <- 1
  if (grepl("Pop", combi[i])==TRUE & gregexpr("year", combi[i])==TRUE){N <- 26*6}
...
result <- foreach(i = 1:ls, .combine = cbind) %do% {
...
pltmodAIC <- combi_plantule[posAIC]
pltmodBIC <- combi_plantule[posBIC]
