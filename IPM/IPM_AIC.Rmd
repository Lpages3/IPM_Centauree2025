...
numbofcap <- function(x, a) {
  ...
  nbcap <- ifelse(sortie2$V1>=0,sortie2$V1,0)
  return(matrix(nbcap, MatrixDim, MatrixDim, byrow = T))
}
...
sx01 <- function(x,a) {
  ...
  sortie <- predict(Survglm11, fake_data, allow.new.levels = T)
  sortie2 <- aggregate(sortie, list(fake_data$Size0Mars), mean)
  return(sortie2$V1)
}
...
sx02 <- function(x, a) {
  fake_data$age0 <- a
  fake_data$Size0Mars <- x
  sortie <- predict(Survglm12, fake_data, allow.new.levels = T)
  sortie2 <- aggregate(sortie, list(fake_data$Size0Mars), mean)
  return(sortie2$V1)
}
...
flr0 <- function(x, a, Beta) {
  fake_data$age0 <- a
  fake_data$Size0Mars <- unique(x)
  sortie <- plogis(predict(Flowglm1, newdata = fake_data, allow.new.levels = T, type = "link") + Beta)
  sortie2 <- aggregate(sortie, list(fake_data$Size0Mars), mean)
  return(sortie2$V1)
}
...
Gyx1 <- function(ysup,yinf,x,a){
  ...
  fake_data$age0 <- a #Fixe l'age
  fake_data$Size0Mars <- unique(x) #Fixe la taille
  sortie <- predict(Growthglm1, fake_data, allow.new.levels = T) #Prédit les taille à t+1 possibles 
  sortie2 <- aggregate(sortie, list(fake_data$Size0Mars), mean) #Calcule la moyenne des taillet+1
  Sdev <- mean(residVar(Growthglm1))
  cdf.sup <- pnorm(ysup, exp(sortie2$V1), exp(Sdev)) 
  cdf.inf <- pnorm(yinf, exp(sortie2$V1), exp(Sdev)) 
...
Gyx2 <- function(ysup,yinf,x,a){
  ...
  fake_data$age0 <- a #Fixe l'age
  fake_data$Size0Mars <- unique(x) #Fixe la taille
  sortie <- predict(Growthglm2, fake_data, allow.new.levels = T) #Prédit les taille à t+1 possibles 
  sortie2 <- aggregate(sortie, list(fake_data$Size0Mars), mean) #Calcule la moyenne des taillet+1
  Sdev <- 1.05+0.54*log(x)
  cdf.sup <- pnorm(ysup, sortie2$V1, Sdev) 
  cdf.inf <- pnorm(yinf, sortie2$V1, Sdev) 
  return(cdf.sup - cdf.inf)
}
ysup <- 0.8
yinf <- 0.5
...
K.fnc0 <- function(age, n.size, estbl, Beta=0){
  ...
  if(age==1){ymid.a <- ymid.1 ; yI.a <- yI.1}
