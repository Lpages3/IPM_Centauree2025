...
combi_surv1 <- combinations #Modelength_combi for seedling survival no age effect
...
combi_int0 <- combi_notInd
for(i in 1:length(combi_notInd)){
  combi_int0[[i]][length(combi_int0[[i]])] <- "0"
}
...
```{r}
(Cptlglm1 <- fitme(log(Capitule) ~ 1 + offset(Size0Mars), 
                  data=cptl_data))
```
...
```{r}
cptl_data <- IPM_data[IPM_data$Flowering==1,]
Cptlglm1 <- fitme(log(Capitule) ~ 1 + Size0Mars + (Age|year), 
              data=cptl_data)
# NbrCptl = exp(2.31070+0.06846*Size0Mars)

cptl_data_predi <- cptl_data %>% 
  mutate(Capitule = ifelse(length_combie(is.na(Capitule), exp(2.31070+0.06846*Size0Mars), Capitule)))
```
